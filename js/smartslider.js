    ...
    $.fn.extend({
        strackbar: function(options) {
            ...
            return this.each(function() {
                ...
                var stepValue = parseFloat(settings.sliderWidth / (settings.maxValue - settings.minValue) );
                ...
                maxvallabel.html(settings.maxValue/10).css('padding-left', '6px'); //.css('margin-top', '-2px');
                minvallabel.html(settings.minValue/10).css('padding-right', '4px'); //.css('margin-top', '-2px');
                ...
                for (var count = 0; count < totalTicks/10; count++) {
                    var tick = $('<li><span>|</span></li>');
                    tick.css('width', stepValue * 10+ 'px');
                    ticks.append(tick);
                }
                if (settings.ticks)
                    slider.after(ticks);
                ...
                function setTrackerPosition(cursorPosition) {
                    ...
                    if (cursorPosition >= sliderLeft && cursorPosition <= (sliderRight - stepValue / 10)) {
                        if (trackerPosition >= 0 && trackerPosition <= settings.sliderWidth - stepValue / 10) {
                            ...
                            if (settings.callback != null)
                                settings.callback(currentValue + settings.minValue);
                        }
